"""`EmitStage`: writes publication artifacts, a dist-git `sources` manifest, and
`report.json` to `/output`.

Zero publication artifacts is non-fatal here (empty manifest, still emits) --
