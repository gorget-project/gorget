...
from gorget.config.schema import ToolchainEntry, VendorModule, VendorPinEntry, VendorPinStep
...
from gorget.transform.vendor_pin import VendorPinHandler, _CargoPin, _GoPin, _NpmPin
...
def test_go_pin_runs_edit_then_tidy(tmp_path, mocker):
    ...
    entry = VendorPinEntry(dependency="golang.org/x/net", minimum_version="0.23.0")
...
def test_go_pin_edit_failure_raises(tmp_path, mocker):
    mocker.patch("gorget.transform.vendor_pin.run", return_value=_fail("bad module"))
    entry = VendorPinEntry(dependency="x", minimum_version="1.0.0")
...
def test_go_pin_tidy_failure_raises(tmp_path, mocker):
    mocker.patch(
        "gorget.transform.vendor_pin.run", side_effect=[_ok(), _fail("tidy broke")]
    )
    entry = VendorPinEntry(dependency="x", minimum_version="1.0.0")
...
def test_npm_pin_edits_dependencies_and_installs(tmp_path, mocker):
    ...
    mock_run = mocker.patch("gorget.transform.vendor_pin.run", return_value=_ok())
    entry = VendorPinEntry(dependency="left-pad", minimum_version="1.3.0")
...
def test_npm_pin_edits_dev_dependencies(tmp_path, mocker):
    ...
    entry = VendorPinEntry(dependency="eslint", minimum_version="9.0.0")
    _NpmPin().apply(tmp_path, entry, [])
    data = json.loads((tmp_path / "package.json").read_text())
...
def test_npm_pin_dependency_not_found_raises(tmp_path):
    ...
    entry = VendorPinEntry(dependency="missing-pkg", minimum_version="1.0.0")
...
def test_npm_pin_install_failure_raises(tmp_path, mocker):
    ...
    mocker.patch("gorget.transform.vendor_pin.run", return_value=_fail("npm error"))
    entry = VendorPinEntry(dependency="x", minimum_version="1.0.0")
...
def test_cargo_pin_edits_toml_and_updates(tmp_path, mocker):
    ...
    mock_run = mocker.patch("gorget.transform.vendor_pin.run", return_value=_ok())
    entry = VendorPinEntry(dependency="serde", minimum_version="1.0.190")
...
def test_cargo_pin_missing_toml_raises(tmp_path):
    entry = VendorPinEntry(dependency="serde", minimum_version="1.0.0")
...
def test_cargo_pin_update_failure_raises(tmp_path, mocker):
    (tmp_path / "Cargo.toml").write_text('[dependencies]\nserde = "1.0.0"\n')
    mocker.patch("gorget.transform.vendor_pin.run", return_value=_fail("cargo error"))
    entry = VendorPinEntry(dependency="serde", minimum_version="1.0.190")
...
def test_handler_applies_pins_per_module(tmp_path, mocker):
    ...
    step = VendorPinStep(
        ecosystem="go",
        pins=[VendorPinEntry(dependency="x", minimum_version="1.0.0")],
...
def test_handler_dry_run_does_nothing(tmp_path, mocker):
    mock_run = mocker.patch("gorget.transform.vendor_pin.run")
