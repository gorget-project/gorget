...
def test_pnpm_vendor_runs_install_per_default_platform(tmp_path, mocker):
    mock_run = mocker.patch("gorget.transform.vendor.pnpm.run", return_value=_ok())
...
def test_pnpm_vendor_raises_on_failure(tmp_path, mocker):
    mocker.patch(
        "gorget.transform.vendor.pnpm.run", return_value=_fail(stderr="ERR_PNPM_OUTDATED")
    )
...
def test_pnpm_vendor_creates_store_dir(tmp_path, mocker):
    mocker.patch("gorget.transform.vendor.pnpm.run", return_value=_ok())
    PnpmVendor().vendor(tmp_path)
