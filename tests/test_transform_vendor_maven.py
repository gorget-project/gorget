...
def test_maven_vendor_populates_project_local_repository(tmp_path, mocker):
    ...
    mock_run = mocker.patch("gorget.transform.vendor.maven.run", return_value=_completed())
...
def test_maven_vendor_reports_command_failure(tmp_path, mocker):
    (tmp_path / "pom.xml").write_text("<project/>")
    mocker.patch(
        "gorget.transform.vendor.maven.run", return_value=_completed(1, "resolution failed")
    )
